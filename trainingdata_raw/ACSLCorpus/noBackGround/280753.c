...
bool all_positive(int *listval, int len) {
    ...
    */
    ...
    //@ assert all_pos ==> (\forall integer j; (0) <= j < (len) ==> (listval)[j] > 0);
    //@ assert (\forall integer j; (0) <= j < (len) ==> (listval)[j] > 0) ==> listval[0] > 0;
    //@ assert (\forall integer j; (0) <= j < (len) ==> (listval)[j] > 0) ==> listval[len-1] > 0;
    return all_pos;
}
