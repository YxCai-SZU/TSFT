...
/*@
    ...
    ensures \result == 1 <==>
        (\exists integer k; 0 <= k < a_len && a[k] == key &&
         (\forall integer j; 0 <= j < a_len && j != k ==> a[j] != key));
    ensures \result == 0 <==>
        ((\forall integer k; 0 <= k < a_len ==> a[k] != key) ||
         (\exists integer k1, k2; 0 <= k1 < k2 < a_len && a[k1] == key && a[k2] == key));
*/
