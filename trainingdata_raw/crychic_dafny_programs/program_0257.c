...
/*@
  ...
  loop invariant \valid_read(str + i + (0 .. str_len - i - 1));
  loop invariant str_len - i >= sub_len || i > j;
  loop assigns i, temp;
  loop variant j - i + 1;
*/
  while ((i <= j))
...
/*@
  ...
  loop invariant \valid_read(str1 + i + (0 .. str1_len - i - 1));
  loop invariant str1_len - i >= k || i >= endIndex;
  loop assigns i, temp;
...
int maxCommonSubstringLength(char* str1, int str1_len, char* str2, int str2_len)
{
  ...
  int i = 1;
/*@
  loop invariant 1 <= i <= str1_len + 1;
  loop invariant 1 <= i <= str2_len + 1;
  loop invariant commonSubstrLength >= 0;
  loop invariant commonSubstrLength < i;
