...
/*@
    ...
    ensures \forall integer i; num < i < a_len ==> a[i] == \old(a[i]);
*/
...
/*@
  ...
  loop invariant \forall integer k; 0 <= k < i ==> a[num - k] == \at(a[k], LoopEntry);
  loop invariant \forall integer k; i <= k <= j ==> a[k] == \at(a[k], LoopEntry);
  loop invariant \forall integer k; j < k <= num ==> a[num - k] == \at(a[k], LoopEntry);
